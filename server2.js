...
try {
    ...
    .then ( result => {
        console.log(result); //s3로부터 받은 데이터
    }
    )
    .catch ( err => { //s3에 데이터가 없으면 api call 하고 해당 데이터 리턴
        const response = axios.get("https://api.fitbit.com/1/user/" + "C2XSWR" + "/activities/date/" + "2024-04-30" +".json", { //user_id,  date
        headers: {
        ...
        //s3에 키가 존재하지 않는 경우 -->  api를 호출하여 데이터를 가져옴
            
            //s3에 동일한 객체명이 없는 경우 s3에 데이터를 저장하고 해당 데이터를 반환
...
function getObject(bucketName, key) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (err) {
            ...
            //console.log(data.Body.toString('utf-8')); //s3로부터 가져온 데이터
        }
...
const checkObject = async (userId, date) => {
    try {
      const object = await getObject(bucketName, `${userId}_${date}.json`)
      .then( result => {
        return result;
    })
    } catch (err) {
  ...
  const fetchActivitySummray = async (accessToken, userId, date) => {
